...
vllm serve /mnt/share/weight/Qwen3-30B-A3B-Instruct-2507/ \
  ...
  --compilation-config '{"cudagraph_capture_sizes":[4, 8, 16, 24], "cudagraph_mode": "FULL_DECODE_ONLY"}' \
  --profiler-config '{"profiler": "torch", "torch_profiler_dir": "./vllm_profile", "torch_profiler_with_stack": false}' \
  --no-enable-prefix-caching \
  --gpu-memory-utilization 0.9 \
  --async-scheduling \
  --additional-config='{"enable_cpu_binding":false}' \
  --kv-transfer-config '{"kv_connector": "MooncakeConnectorV1", "kv_role": "kv_consumer", "kv_port": "30400", "engine_id": "2",
